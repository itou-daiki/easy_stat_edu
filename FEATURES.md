...
**機能:**
- `kuromoji.js`による高度な日本語形態素解析
  - 品詞（名詞, 動詞, 形容詞）によるフィルタリング
  - 単語の基本形（原型）への正規化
- 頻出単語の抽出と出現回数のカウント

**可視化:**
- ワードクラウドによる直感的な特徴語の可視化
- 頻出単語のランキング表（Top 20）

**用途:**
- テキストデータの全体像の把握
- 文章に含まれる主要なキーワードやテーマの発見
- アンケートの自由回答などの分析

**注意:**
- すべての処理はブラウザ内で完結し、外部サーバーにデータを送信しません。
- 初回実行時には、形態素解析用の辞書ファイルを読み込むため時間がかかる場合があります。
