...
def get_eda_summary():
    ...
    """
    if common.current_df is None:
        return "<p>データが読み込まれていません</p>"

    # Basic info
    n_rows, n_cols = common.current_df.shape

    # Summary statistics
    summary_df = common.current_df.describe(include='all').transpose()
    ...
    <div class="data-summary">
        ...
        <p>列数: {n_cols}</p>
        <h4>データプレビュー</h4>
        {common.current_df.head(10).to_html(classes='table table-striped table-hover')}
        <h4>要約統計量</h4>
...
def get_variable_plots():
    ...
    categorical_cols = common.get_categorical_columns()
    numerical_cols = common.get_numeric_columns()

    # Categorical variables
    for col in categorical_cols:
        value_counts = common.current_df[col].value_counts()
        fig = px.bar(
    ...
    for col in numerical_cols:
        fig = px.histogram(common.current_df, x=col, title=f'【{col}】 の可視化（ヒストグラム）')
        fig.update_layout(bargap=0.2)
        html += fig.to_html(full_html=False, include_plotlyjs='cdn')

        fig = px.box(common.current_df, y=col, title=f'【{col}】 の可視化（箱ひげ図）')
        html += fig.to_html(full_html=False, include_plotlyjs='cdn')
...
def get_two_variable_plot(var1, var2):
    ...
    if var1 in categorical_cols and var2 in categorical_cols:
        cross_tab = pd.crosstab(common.current_df[var1], common.current_df[var2])
        fig = px.imshow(
            cross_tab,
    ...
    # 数値×数値
        ...
        fig = px.scatter(common.current_df, x=var1, y=var2, title=f'散布図： 【{var1}】 × 【{var2}】')
    # カテゴリ×数値
    else:
        ...
        fig = px.box(common.current_df, x=cat_var, y=num_var, title=f'箱ひげ図： 【{cat_var}】 × 【{num_var}】')
...
def get_three_variable_plot(cat_var1, cat_var2, num_var):
    ...
    # データの準備
    grouped_df = common.current_df.groupby([cat_var1, cat_var2])[num_var].mean().reset_index()
