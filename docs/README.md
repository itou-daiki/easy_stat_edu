...
ブラウザ上で動作する統計分析Webアプリケーション（PyScript版）

## 概要

easyStatは、Pythonの統計ライブラリをブラウザ上で実行できるようにしたWebアプリケーションです。サーバー不要で、GitHub Pagesなどの静的ホスティングサービスで動作します。

**特徴:**
- 📊 ブラウザ上でPython統計ライブラリを実行
- 🔒 データはブラウザ内で処理（サーバーに送信されません）
- 💻 サーバー不要（完全な静的サイト）
- 🌐 GitHub Pagesで簡単にデプロイ可能
